...
- 2026-09-11 verification: `/Users/aa/Downloads/person_skinned.morphpack` compiles to a 19,327-byte schema 2 pack from `/Users/aa/Downloads/person_skinned.glb` and is ready for hash-addressed runtime placement. Rust has 144 passing tests, Studio has 20 passing tests, and both normal and Studio builds pass. Cross-target Android-feature verification remains required after the shared renderer change; the browser WASM target is still unavailable locally.
- 2026-09-11: Added the first authored clothing slice for the canonical Person base. Schema 2 top, bottom, and footwear packs use the same 15-joint skin contract; Studio resolves their occupied slots independently, and the renderer suppresses only the covered procedural shirt, pants, or shoes so authored and compatibility assets can be mixed during migration.
- 2026-09-11 verification: `person_top.morphpack` (7,692 bytes), `person_bottom.morphpack` (6,418 bytes), and `person_shoes.morphpack` (3,834 bytes) validate and are staged under `foo/runtime/morphs/sha256`. Rust tests (144), Studio tests (20), backend tests (10), Android-feature checks, web-renderer checks, and Studio release checks pass. The full Phase 5 exit criterion remains open until a real Blender-authored garment is validated through the motion/clipping suite on every renderer backend.
- 2026-09-11: Replaced the first cube blockouts after live visual review. The generated Person source now uses the engine's actual biped rest hierarchy, fitted under-clothing dimensions, and the same rounded head/torso/sleeve/shorts/shoe profiles as the established character silhouette. Authored base, top, bottom, and footwear materials now inherit the live appearance skin, shirt, pants, and shoe colors. This restores analytic face placement and prevents the base mesh from covering polished garment layers while retaining the schema 2 skinned-pack path.
- 2026-09-11 verification: the refined base/top/bottom/footwear packs validate at 352,881, 63,277, 42,609, and 42,617 bytes respectively. Their Near/Mid/Far triangle counts remain bounded at 7,480/2,856/816 for the base and at most 1,320/504/144 for a clothing pack. Rust tests (144), Studio tests (20), backend tests (10), Android-feature checks, web-renderer checks, Studio release checks, migration replay, formatting, and diff checks pass.
- 2026-09-11: Refined Person Top into an authored hoodie fixture with a skinned open hood shell and front drawstrings while retaining the single triangle-list primitive required by the morph pack contract. The replacement pack is 173,438 bytes with 3,528/1,416/432 Near/Mid/Far triangles, hash `cf9633fa4a4335f8c73a7d73e932950e00f3e4d17c2988f60555b64b32bcc4b7`, and migration `019_add_person_hoodie_details.sql` updates the local/production catalog reference.
- 2026-09-11: Replaced that visual-review blockout with the established lowered-hood profile, a shallow kangaroo pocket, and curved white drawstrings that emerge at the neckline and follow the chest. Bounded schema 3 packs now support up to eight contiguous material surfaces per LOD while schema 1/2 decoding stays compatible; surfaces can either inherit the live avatar tint or preserve an authored color without changing the normal character vertex path. The refined Person Top pack is 171,240 bytes with 3,056/1,512/744 Near/Mid/Far triangles, hash `f332320644fab2c390bc89b823e74ae9c9d48a1965abaa0b3980d6dc6dd52a8a`, and migration `020_refine_person_hoodie_materials.sql` updates the catalog reference. Person eyes now sit on the front-side head curve so the near eye remains readable while orbiting toward profile.
- 2026-09-11: Restored the established Person shoe detail in the authored footwear pack. The rounded navy upper, thin ivory outsole, and three raised ivory lace bands are separate bounded schema 3 surfaces at every LOD, so selecting Person Shoes no longer replaces the detailed legacy footwear with one flat avatar-tinted shape. The pack is 136,246 bytes with 2,768/960/480 Near/Mid/Far triangles, hash `c2e132da1e5e1362e122594506111c72a13aec1fbba8c29794e8b9650f8ddf3b`, and migration `021_refine_person_shoe_details.sql` updates the catalog reference.
- 2026-09-11: Rebuilt the authored hoodie pocket from the established tapered profile instead of a generic rounded pebble. Modeled dark diagonal hand openings and four ivory reinforcement stitches restore the original pocket hierarchy while remaining three bounded material surfaces per LOD. The Person Top pack is 190,100 bytes with 3,280/1,680/884 Near/Mid/Far triangles, hash `311e90b0dbc6cf5a12a31c6eef67605c9a2cfa9fd3fbb7095e8ad088e642b22f`, and migration `022_refine_person_hoodie_pocket.sql` updates the catalog reference.
- 2026-09-11: Expanded each authored clothing category to two catalog options and renamed the defaults to Hoodie, Jeans, and Sneakers. Short Sleeve Collared adds a reusable Cubacadabra chest-logo image, Slacks adds modeled pleats and pressed seams, and Sparkles adds bounded sequin, gold-sole, and gold-lace surfaces. Schema 4 packs carry up to four bounded 256px RGBA8 textures plus per-vertex UVs while preserving schema 1-3 decoding. Textured surfaces use a separate GPU pipeline, so untextured characters retain the original shader cost. D1 records source-image metadata and its material usage, R2 stores the immutable original PNG, and the compiled pack embeds its own resized runtime copy. Migration `023_expand_authored_clothing_collection.sql` publishes all six final packs.
- 2026-09-11 verification: deterministic regeneration reproduces all six pack bytes. Hoodie, Jeans, Sneakers, Short Sleeve Collared, Slacks, and Sparkles validate at 190,096, 42,601, 136,242, 352,800, 105,782, and 189,337 bytes. Local D1 migration replay produces 42 assets, one reusable image, and one material binding; all six API pack responses and the original logo response match their catalog hashes. The shared engine has 145 passing tests, Studio has 21, morph authoring has 10, portable morphs has 24, and backend has 12. Host Android/web feature checks and the Studio release build pass. Installed Android/WASM target checks remain unavailable because the active Rust toolchain cannot find their `core` libraries, and the pre-existing Phase 3 world capture still fails before character drawing because its world pipeline omits bind group 1.
- 2026-09-11: Rebuilt Short Sleeve Collared after visual review to follow a fitted polo reference: fixed royal-blue cloth, compact overlapping navy collar leaves, a narrow inset placket, dark fitted sleeve cuffs, two garnet buttons, and a smaller chest logo. Collar, placket, button, and decal points are fitted to the curved torso so they remain attached in profile views. The final pack is 364,631 bytes with 1,736/660/290 Near/Mid/Far triangles, hash `60304e0d891d04b8240b74ed9d639fbde01c905d1f258013cc0e52cc7493dc6a`; migrations `024_refine_short_sleeve_collared.sql` and `025_fit_polo_surface_details.sql` update the immutable catalog reference.

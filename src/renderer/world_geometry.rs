...
fn add_interaction_zone(
    vertices: &mut Vec<Vertex>,
    zone: &RenderInteraction,
    state: crate::types::InteractionRenderState,
    elapsed: f32,
    index: usize,
    palette: RenderPalette,
) {
    let origin = Vec3::from_array(zone.position);
    let pulse = 1.0 + (elapsed * 2.4 + index as f32 * 1.9).sin() * 0.06;
    let color = faded(zone.color, if state.inside { 0.86 } else { 0.52 });
    add_cylinder(
        vertices,
        origin + Vec3::new(0.0, 0.10, 0.0),
        zone.radius + 0.28,
        0.20,
        faded(palette.ink, 0.92),
    );
    add_cylinder(
        vertices,
        origin + Vec3::new(0.0, 0.22, 0.0),
        zone.radius,
        0.035,
        color,
    );
    add_ring(
        vertices,
        origin + Vec3::new(0.0, 0.27, 0.0),
        (zone.radius - 0.18).max(0.35) * pulse,
        0.12,
        color,
    );

    let crystal_color = if state.players > 1 {
        zone.color
    } else {
        faded(zone.color, 0.72)
    };
    for crystal_index in 0..3 {
        let angle = elapsed * 0.34 + crystal_index as f32 * std::f32::consts::TAU / 3.0;
        let radius = 0.72 + (index as f32 * 0.12) + if state.inside { 0.08 } else { 0.0 };
        let height = 0.62 + (crystal_index as f32 * 0.13);
        let transform = Mat4::from_translation(
            origin
                + Vec3::new(
                    angle.cos() * radius,
                    0.68 + (elapsed * 1.8 + crystal_index as f32).sin() * 0.08,
                    angle.sin() * radius,
                ),
    ...
    add_sphere(
        vertices,
        origin + Vec3::new(0.0, 1.28 + (elapsed * 2.0 + index as f32).sin() * 0.10, 0.0),
        0.22 * pulse,
        faded(zone.color, if state.inside { 0.95 } else { 0.78 }),
    );
    add_pixel_text(
        vertices,
        &zone.label,
        origin + Vec3::new(0.0, 1.72, -0.06),
        0.0,
        3.0,
        palette.paper,
    );

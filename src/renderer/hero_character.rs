    ...
    for p in parts.iter_mut() {
        ...
        match (joint, p.tint, p.feature) {
            ...
            (_, _, Feature::Eye(side)) => {
                ...
                p.anchor.local = Mat4::from_translation(Vec3::new(side * 0.265, 0.015, -0.414))
                    * Mat4::from_rotation_y(-side * 0.67);
            }
...
mod tests {
    ...
    fn person_eyes_wrap_onto_the_profile_curve() {
        ...
        for eye in eyes {
            ...
            assert!((0.26..=0.27).contains(&center.x.abs()));
            assert!(center.z < -0.41);
            let normal = eye.anchor.local.transform_vector3(Vec3::Z).normalize();
            assert!(normal.x.abs() > 0.61);
        }
    }

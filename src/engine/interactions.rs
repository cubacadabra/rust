...
impl Engine {
    ...
    /// Returns an authored interaction's world position for host-side
    /// presentation helpers. The host may use this to navigate a demo avatar,
    /// but interaction semantics remain owned by the engine and Luau.
    pub fn interaction_position(&self, index: usize) -> Option<[f32; 3]> {
        self.interactions.world.get(index).map(|zone| zone.position)
    }

    /// Reports whether the local player is currently inside an authored
    /// interaction. This is a read-only presentation signal; enter/exit
    /// events are still generated by the normal interaction runtime.
    pub fn interaction_inside(&self, index: usize) -> bool {
        self.interactions
            .states
            .get(index)
            .is_some_and(|state| state.inside)
    }
